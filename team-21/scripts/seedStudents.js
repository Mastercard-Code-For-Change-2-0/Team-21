...
const dummyStudents = [
  {
    ...
    marksheets: [
      ...
      {
        document_type: "12th",
        file_url: "https://example.com/docs/john_12th.pdf",
        file_name: "john_12th_marksheet.pdf",
        upload_date: new Date("2023-02-01"),
        verified: true
      }
    ]
  ...
  },
  {
    ...
    marksheets: [
      ...
      },
      {
        ...
        file_url: "https://example.com/docs/rahul_diploma.pdf",
        file_name: "rahul_diploma_certificate.pdf",
        upload_date: new Date("2023-02-10"),
        verified: true
      }
    ]
  ...
  },
  {
    ...
    marksheets: [
      ...
      {
        document_type: "12th",
        file_url: "https://example.com/docs/arjun_12th.pdf",
        file_name: "arjun_12th_marksheet.pdf",
        upload_date: new Date("2023-03-05"),
        verified: true
      }
    ]
...
export async function seedStudentData() {
  try {
    ...
    // Insert dummy data
    const insertedStudents = await User.insertMany(dummyStudents);
    console.log(`✅ Successfully inserted ${insertedStudents.length} students`);

    // Log summary
    const placedCount = insertedStudents.filter(s => s.placement_status === 'Placed').length;
    const multipleOffersCount = insertedStudents.filter(s => s.placement_status === 'Multiple Offers').length;
    const notPlacedCount = insertedStudents.filter(s => s.placement_status === 'Not Placed').length;

    console.log('\n📊 Summary:');
    console.log(`👨‍🎓 Total Students: ${insertedStudents.length}`);
    console.log(`✅ Placed: ${placedCount}`);
    console.log(`🌟 Multiple Offers: ${multipleOffersCount}`);
